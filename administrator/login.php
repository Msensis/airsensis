<?php 
	...
	function login($username,$password){
		...
		if (!($stmt = $mysql->prepare("select id, name, role from user where role=? and username=? and AES_DECRYPT(password, UNHEX(SHA2(?,512)))=?"))) {
//			 throw new Exception("Prepare failed: (" . $mysql->errno . ") " . $mysql->error);
			 $msg = $msg_general_error;
			 return false;
		}
		$admin_role = 1;
		if (!$stmt->bind_param("isss", $admin_role, $username, $secret, $password)) {
	...
	<div class="page-wrapper">
		<div class="container main-container">
			...
			<div class="content-area">
				<div class="title"><h3><?php echo $admin_login_title;?></h3></div>
